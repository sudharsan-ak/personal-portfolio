...
export default function APIPage() {
  const endpoints: ApiEndpoint[] = [
    ...
    { title: "SHA256 Hash Generator", description: "Send text and receive its SHA256 hash.", path: "/api/hash", copyKey: "hash", type: "input" },
    { title: "Word Counter", description: "Counts the number of words in the text you provide.", path: "/api/word-count", copyKey: "words", type: "input" },
    { title: "Character Counter", description: "Counts the number of characters in the text you provide.", path: "/api/char-count", copyKey: "characters", type: "input" },
      ...
      if (type === "input") {
        ...
        res = await fetch(path, {
          ...
          body: JSON.stringify({
            ...
            hour: tzInput.hour || 12,
            minute: tzInput.minute || 0,
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto relative">
      <h1 className="text-4xl font-bold mb-6 text-center text-white">Public REST API</h1>
              ...
              <div className="p-4 bg-gray-800 text-gray-200">
                ...
                {endpoint.type === "timezone" && (
                  <div className="flex flex-col sm:flex-row sm:gap-4 gap-2 mb-4 items-center">
                    <select value={inputs[endpoint.path]?.fromTimezone || "UTC"}
                            onChange={(e) => setInputs(prev => ({ ...prev, [endpoint.path]: { ...prev[endpoint.path], fromTimezone: e.target.value } }))}
                            className="px-4 py-2 rounded bg-gray-900 text-white border border-gray-700">
                      {timezones.map(tz => <option key={tz} value={tz}>{tz}</option>)}
                    </select>
                    <input type="number" min={1} max={12} placeholder="Hour"
                           value={inputs[endpoint.path]?.hour || ""}
                           onChange={(e) => setInputs(prev => ({ ...prev, [endpoint.path]: { ...prev[endpoint.path], hour: e.target.value } }))}
                           className="px-4 py-2 rounded bg-gray-900 text-white border border-gray-700 w-20" />
                    <input type="number" min={0} max={59} placeholder="Minute"
                           value={inputs[endpoint.path]?.minute || ""}
                           onChange={(e) => setInputs(prev => ({ ...prev, [endpoint.path]: { ...prev[endpoint.path], minute: e.target.value } }))}
                           className="px-4 py-2 rounded bg-gray-900 text-white border border-gray-700 w-20" />
                    <select value={inputs[endpoint.path]?.ampm || "AM"}
                            onChange={(e) => setInputs(prev => ({ ...prev, [endpoint.path]: { ...prev[endpoint.path], ampm: e.target.value } }))}
                            className="px-4 py-2 rounded bg-gray-900 text-white border border-gray-700">
                      <option value="AM">AM</option>
                      <option value="PM">PM</option>
                    </select>
                    <select value={inputs[endpoint.path]?.toTimezone || "UTC"}
                            onChange={(e) => setInputs(prev => ({ ...prev, [endpoint.path]: { ...prev[endpoint.path], toTimezone: e.target.value } }))}
                            className="px-4 py-2 rounded bg-gray-900 text-white border border-gray-700">
                      {timezones.map(tz => <option key={tz} value={tz}>{tz}</option>)}
                    </select>
                  </div>
                ...
                <div className="flex flex-col sm:flex-row sm:gap-4 gap-2 mb-4">
                  <button onClick={() => fetchData(endpoint.path, endpoint.type)}
                          className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded transition-colors duration-200 w-full sm:w-auto">Try It</button>
                  {endpoint.copyKey && response && (
                    <button onClick={() => copyData(endpoint.path, endpoint.copyKey!)}
                            className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded transition-colors duration-200 w-full sm:w-auto">Copy {endpoint.copyKey}</button>
                  )}
                </div>
      ...
      {toast && (
        <div className="fixed bottom-6 right-6 bg-gray-900 text-white px-4 py-2 rounded shadow-lg animate-fade-in">{toast}</div>
      )}
